...
import "../../Style/Admin/danhsachdangkytiem.css"
...
const Danhsachdatiem = (props) => {
  ...
  const arrayPhanTrang = ()=>{
    ...
    if(listComfirm)
    {
          for(let i=1;i<=listComfirm.totalPages;i++){
                  Array.push(i);
          } 
    }
    return Array;
  ...
  const handlePrevious = () =>{
    const currentPage = listComfirm.currentPage 
     if(currentPage===0 || listComfirm.totalPages===0){
         return;
     }
     getDataComfirm(currentPage-1);
   
   }
   const handleNext = () =>{
    const currentPage = listComfirm.currentPage 
    const totalPages = listComfirm.totalPages;
    if(currentPage >=totalPages-1 )
      return;
      getDataComfirm(currentPage+1);
  }
  const handleChoosePage = (_page) =>{  
    getDataComfirm(_page);
  }

  const getDataComfirm = (_page) =>{
    InjectionregistrationService.getComfirm(_page)
    .then((response) =>{
      setListComfirm(response.data);
    ...
    <div className="container1">
      ...
      <div className="table-vacxin1">
        <Table id="customer1">
          ...
          <tbody>
          {
            listComfirm && listComfirm.listnews.map((ele,index)=>(
              <tr key = {index}>
                <td>{index +1}</td>
      ...
      <nav aria-label="Page navigation example">
          <ul className="pagination justify-content-center">
            ...
            <li className={ listComfirm&& (listComfirm.currentPage===0 ||listComfirm.totalPages===0)? "disabled page-item" :"page-item" } onClick={handlePrevious}>
              <a className="page-link" >Previous</a>
            </li>
              {
                arrayPhanTrang().map((ele)=>
                  <li className={listComfirm&& listComfirm.currentPage === ele-1 ?"page-item active" : "page-item"}
                  onClick={() =>{handleChoosePage(ele-1)}}><a className="page-link ">{ele}</a></li>
                )

              }
            <li className={ listComfirm && (listComfirm.currentPage>=listComfirm.totalPages-1 )? "disabled page-item" :"page-item" } onClick={handleNext}>
